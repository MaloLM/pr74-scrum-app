...
namespace pr74_scrum_app
{
    class SidebarController
    {
        ...
        public Dictionary<int,string> GetProjects(int userID, bool pinned)
        {
            ...
            string sql = $"select * from Member m inner join Project p on m.Project_id = p.id where m.pinned = {pinned} and user_id='{userID}' and p.archived=0 order by p.id DESC LIMIT 5";
